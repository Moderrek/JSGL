...
export class Game {
    ...
    private _gameLoopUpdate(time: number) {
        ...
        this.input.mouseScrollDelta = new Vector2(
            0,
            this._tempMouseWheelDeltaY,
        );
    ...
    StartGameLoop() {
        if (this._isPlaying) {
            console.warn(
                'Cannot start new game loop when the game loop exists.',
            );
            return;
        }
    ...
    LoadAllResources() {
        ...
        this.resources.forEach(resource => {
            ...
            if (resource.type === 'image') {
                ...
                // Unknown type
                console.warn(
                    `Type "${resource.type}" is unknown resources type.`,
                );
                return;
            }
            resourcesCount += 1;
    ...
    public get mouseHoveredGameObject(): ClickableGameObject | undefined {
        for (let i = this.gameObjects.length - 1; i >= 0; i -= 1) {
            ...
            const maxX =
                clickableObj.transform.position.x +
                clickableObj.transform.scale.x;
            const minY = clickableObj.transform.position.y;
            const maxY =
                clickableObj.transform.position.y +
                clickableObj.transform.scale.y;
            const isInRange =
